...
impl ElectrumSession {
    ...
    pub fn start_threads(&mut self) -> Result<(), Error> {
        ...
        info!("login STATUS block:{:?}", self.block_status()?);
                ...
                match syncer.sync(&client, &mut last_statuses, first_sync_bool) {
                    Ok(sync_result) => {
                        ...
                        for pointer in sync_result.accounts {
                            let mut new_account_added = false;
                            if syncer.synced_accounts.insert(pointer) {
                                ...
                                new_account_added = true;
                            }
                            if new_account_added {
                                notify.subaccount_synced(
                                    syncer.synced_accounts.iter().copied().collect(),
                                );
                            }
                        }
                    }
    ...
    pub fn get_unspent_outputs(&self, opt: &GetUnspentOpt) -> Result<GetUnspentOutputs, Error> {
        let mut unspent_outputs: HashMap<Option<elements::AssetId>, Vec<UnspentOutput>> =
            HashMap::new();
...
impl Syncer {
    ...
    fn ntf_satoshi_type(
        ...
        if self.network.liquid {
            ...
            let balance = balances.get(&None).expect("bitcoin balance always has btc key");
