...
use gdk_common::elements::confidential::{AssetBlindingFactor, ValueBlindingFactor};
...
impl Account {
    ...
    pub fn get_previous_addresses(
        ...
        for index in (end..before_pointer).rev() {
            let (address, public_key) = self.derive_address_with_public_key(is_internal, index)?;
            let script_pubkey = address.script_pubkey();
            let account_path =
    ...
    pub fn list_tx(&self, opt: &GetTransactionsOpt) -> Result<Vec<TxListItem>, Error> {
        ...
        let mut my_txids: Vec<(&BETxid, &Option<u32>)> = acc_store
            ...
            .filter(|(_, height)| {
                !opt.pending_only
                    || height.map(|height| height >= pending_tip_height).unwrap_or(true)
            })
            .collect();
        ...
        for (tx_id, height) in my_txids.iter() {
                    ...
                    let (asset_id, asset_blinder, amount_blinder) = {
                        if let BEOutPoint::Elements(outpoint) = beoutpoint {
                            (
                                ...
                                acc_store.all_txs.get_previous_output_assetblinder(
                                    *outpoint,
                                    &acc_store.unblinded,
                                ),
                                acc_store.all_txs.get_previous_output_amountblinder(
                                    *outpoint,
                                    &acc_store.unblinded,
                                ),
                            )
            ...
            let outputs = (0..tx.output_len() as u32)
                .map(|vout| {
                    ...
                    let asset_id = tx.output_asset(vout, &acc_store.unblinded).map(|a| a);
                    let asset_blinder = tx.output_assetblinder(vout, &acc_store.unblinded);
                    let amount_blinder = tx.output_amountblinder(vout, &acc_store.unblinded);
                    let is_blinded = is_blinded(&asset_blinder, &amount_blinder);
        ...
        info!("list_tx {:?}", txs.iter().map(|e| &e.txhash).collect::<Vec<&BETxid>>());
...
fn is_blinded(
    asset_blinder_hex: &Option<AssetBlindingFactor>,
    amount_blinder_hex: &Option<ValueBlindingFactor>,
) -> Option<bool> {
    match (asset_blinder_hex, amount_blinder_hex) {
        (Some(abf), Some(vbf)) => {
            Some(*abf != AssetBlindingFactor::zero() || *vbf != ValueBlindingFactor::zero())
        }
        _ => None,
    }
}
